...
public class ProductoServiceImpl implements ProductoService {
    ...
    public List<ProductoDTO> listarTodos() {
        log.info("[CACHE MISS] listarTodos productos — consultando BD");
    ...
    public ProductoDTO obtenerPorId(Long id) {
        log.info("[CACHE MISS] obtenerProducto id={} — consultando BD", id);
        return toDTO(productoRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Producto no encontrado con id: " + id)));
    }

    @Override
    @Cacheable(value = "producto", key = "'sku-' + #sku.toUpperCase()")
    ...
    @Override
    @Transactional
    @CacheEvict(value = {"productos", "producto", "bajo-critico"}, allEntries = true)
    public ProductoDTO crear(ProductoDTO dto) {
        ...
        log.info("[CACHE EVICT] productos — nuevo producto id={}", guardado.getId());
    ...
    @Override
    @Cacheable(value = "productos", key = "'marca-' + #marcaId")
    ...
    @Override
    @Cacheable(value = "bajo-critico", key = "'lista'")
